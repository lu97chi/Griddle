...
var _lodash2 = _interopRequireDefault(require("lodash.assignin"));
...
var _lodash6 = _interopRequireDefault(require("lodash.flow"));

var _lodash7 = _interopRequireDefault(require("lodash.flowright"));
...
function extendArray(objects) {
  ...
  var combinedObject = _lodash2.default.apply(this, objects); //TODO: why are we doing this? is it necessary
...
function getObjectWherePropertyEndsWith(ending, object) {
  ...
  return (0, _lodash3.default)(object, keys);
}
...
function getKeysForObjects(objects) {
  return (0, _lodash5.default)((0, _lodash4.default)(objects.map(function (o) {
...
function removeHooksFromObject(reducerObject) {
  return (0, _lodash.default)(reducerObject, function (value, key) {
...
function getBeforeHooksFromObject(reducerObject) {
  return removeKeyNamePartFromObject((0, _lodash.default)(reducerObject, function (value, key) {
...
function buildGriddleReducerObject(reducerObjects) {
  ...
  if (reducerObjects.length > 0) {
    ...
    for (var key in reducerObjects) {
      var reducer = (0, _lodash.default)(reducerObjects[key], function (value, key) {
        return (0, _lodash8.default)(key);
      });
...
function callReducerWithBeforeAfterPipe(reducerObject, state, action) {
  ...
  var method = (0, _lodash6.default)([before, partialCall, after]);
  return method(state);
}
...
function getReducersByWordEnding(reducers, ending) {
  return reducers.reduce(function (previous, current) {
    ...
    var reducer = (0, _lodash3.default)(current, keys); //TODO: clean this up it's a bit hacky
    ...
    for (var key in current) {
      ...
      var keyWithoutEnding = key.replace("_".concat(ending), ""); //make a new method that pipes output of previous into state of current
  ...
  return componentArray.reduce(function (previous, current) {
    ...
    for (var key in current) {
      ...
      if (key.endsWith(wordEnding) && (previous.hasOwnProperty(keyWithoutEnhancer) || current.hasOwnProperty(keyWithoutEnhancer))) {
        ...
        (0, _lodash7.default)(current[key], current[keyWithoutEnhancer] || previous[keyWithoutEnhancer]) : // Wrap the current component in the Enhancer or container
        current[key](current[keyWithoutEnhancer] || previous[keyWithoutEnhancer]);
      }
    }

    return (0, _lodash.default)(Object.assign(previous, current, newObject), function (v, k) {
